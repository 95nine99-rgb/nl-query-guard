...
public class RawSqlDebugController {
    ...
    private final UserIdEnforcer userIdEnforcer;
    ...
    public RawSqlDebugController(@Qualifier("appJdbcTemplate") JdbcTemplate appJdbc,
        ...
        this.userIdEnforcer = enforcer;
    }

    public record RawSqlRequest(String sql, String account, Boolean guarded) {
    }
    ...
    public RawSqlResult run(@RequestBody RawSqlRequest request) {
        ...
        try {
            if (Boolean.TRUE.equals(request.guarded)) {
                sql = userIdEnforcer.enforce(sql, 1L);
            }
            ...
            return new RawSqlResult(account, false, "GUARD_BLOCKED", e.getMessage(), null, null);
        } catch (Exception e) {
